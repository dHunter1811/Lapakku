...
@section('content')
<div class="container">
    @if(isset($lahan) && ($lahan->status === 'Disetujui' || (Auth::check() && (Auth::id() == $lahan->user_id || Auth::user()->role == 'admin'))))
    <div class="card">
        {{-- Grid utama untuk layout 2 kolom --}}
        <div class="lahan-detail-container">
            {{-- Kolom Kiri: Gambar Utama, Deskripsi, Keuntungan --}}
            ...
            {{-- Kolom Kanan: Harga, Galeri, Info Pemilik, Aksi --}}
                ...
                <div class="galeri-grid-sidebar">
                    ...
                    @if(!empty($galeriImages))
                        ...
                        @for ($i = count($galeriImages); $i < 3; $i++)
                            <div class="galeri-placeholder">Galeri {{ $i + 1 }}</div>
                        @endfor
                    @else
                        @for ($i = 0; $i < 3; $i++)
                             <div class="galeri-placeholder">Galeri {{ $i + 1 }}</div>
                        @endfor
                        <p class="text-muted-small text-center" style="grid-column: 1 / -1;">Tidak ada gambar galeri.</p>
                    @endif
                </div>

                <h4 class="sidebar-subtitle">Lokasi di Peta</h4>
                <div class="map-placeholder">
                    [Peta Interaktif di sini]
                </div>

                <h4 class="sidebar-subtitle">Kontak Pemilik</h4>
                <div class="kontak-pemilik-info">
                    @if($lahan->user)
                    <p><strong>Nama:</strong> {{ $lahan->user->name }}</p>
                    <p><strong>Email:</strong> {{ $lahan->user->email }}</p>
                    <p><strong>No. Telepon:</strong> {{ $lahan->user->no_telepon ?? '-' }}</p>
                    @else
                    <p class="text-muted">Informasi pemilik tidak tersedia.</p>
                    @endif
                </div>

                @auth
                    @if(Auth::id() !== $lahan->user_id)
                        <button type="button" class="btn btn-primary btn-block btn-ajukan-sewa" onclick="openAjukanSewaModal()">
                    ...
                    @endif
                @else
                    <a href="{{ route('login', ['redirect' => url()->current()]) }}" class="btn btn-primary btn-block btn-ajukan-sewa">
        ...
        </div> {{-- Akhir dari lahan-detail-container --}}

        {{-- Bagian Rating & Ulasan (di luar grid 2 kolom utama) --}}
                ...
                <form action="{{ route('lahan.ratings.store', $lahan->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="rating" id="rating" class="form-control" required>
                            ...
                            @for ($i = 5; $i >= 1; $i--)
                            <option value="{{ $i }}">{{ str_repeat('⭐', $i) }} ({{ $i }} Bintang{{ $i > 1 ? 's' : '' }})</option>
                            @endfor
                        </select>
                    </div>
                    <div class="form-group">
    ...
    {{-- Modal untuk Ajukan Sewa (tetap di luar .card utama) --}}
    @auth
...
<style>
    .lahan-detail-container {
        ...
        gap: 30px; /* Jarak antar kolom utama dan sidebar */
    }
    @media (min-width: 992px) { /* Untuk layar besar (lg), buat 2 kolom */
        .lahan-detail-container {
            grid-template-columns: minmax(0, 2.5fr) minmax(0, 1.5fr); /* Kolom kiri lebih lebar, rasio 2.5:1.5 */
        }
    }
    .lahan-detail-main {
        /* Style khusus untuk kolom kiri jika perlu */
    }
    .lahan-detail-sidebar {
        background-color: #f9f9f9;
        padding: 25px; /* Padding sedikit ditambah */
        border-radius: 8px;
        border: 1px solid #e7e7e7;
        height: fit-content; /* Agar tinggi sidebar pas dengan kontennya */
    }
    .lahan-gambar-utama {
        width: 100%;
        max-height: 450px; /* Bisa disesuaikan */
        object-fit: cover;
        border-radius: 8px;
        margin-bottom: 20px;
        border: 1px solid #e0e0e0; /* Border halus pada gambar utama */
    }
    .section-title {
        color: #00796B;
        margin-top: 25px;
        margin-bottom: 10px;
        font-size: 1.35em; /* Ukuran judul section sedikit lebih besar */
        padding-bottom: 5px;
    ...
    .text-muted-small { font-size:0.9em; color:#777; text-align:center; margin-top:5px; }
    ...
    .btn-block { width:100%; text-align:center; } /* Untuk tombol full width */
    ...
    /* Modal styles (seperti sebelumnya) */
...
@push('scripts')
{{-- (script Lightbox2 dan modal tetap sama seperti sebelumnya) --}}
...
<script>
    ...
    function openAjukanSewaModal() {
        if(ajukanSewaModal) {
            ajukanSewaModal.style.display = 'block';
            hitungTotalHarga();
        }
    }
    function closeAjukanSewaModal() {
        if(ajukanSewaModal) {
        ...
        if (hargaPerBulanInput && durasiSewaInput && estimasiTotalHargaSpan) {
            ...
            if (!isNaN(harga) && !isNaN(durasi) && durasi >= 1) {
                ...
                estimasiTotalHargaSpan.textContent = 'Rp 0';
                 if (durasiSewaInput.value < 1 && durasiSewaInput.value !== "") durasiSewaInput.value = 1;
            }
        }
    }
